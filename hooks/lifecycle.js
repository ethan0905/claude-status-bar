...
function run() {
  ...
  if (event === "start") {
    ...
    try {
      ...
      writeAtomic(statePath, { state: "idle", label: "", tool: "", project: cwd ? path.basename(cwd) : "", cwd, project_path: cwd ? cwd.replace(os.homedir(), "~") : "", sessionId: id, transcript: "", entrypoint: process.env.CLAUDE_CODE_ENTRYPOINT || "", term_program: process.env.TERM_PROGRAM || "", pid: process.ppid, started: false, tty: ttyDev(), tmux: !!process.env.TMUX, startedAt: 0, ts: Math.floor(Date.now() / 1000) });
    } catch {}
